...
- Single audio session: starting a new prayer or Rosary step stops the current one. Rosary session state lives in `RosarySessionController` (survives navigation / mini-player). Speech is `SpeechManager.shared`. Settings → Speech → Voice is **Apple Voice** (built-in `AVSpeechSynthesizer`, the old English (US) option) or a bundled ElevenLabs `VoiceBank` clip. System TTS is also the fallback if a clip is missing. Decade announces speak `1st Decade` through `5th Decade`. Scripture citations speak the full book name, chapter, and verses; verse bodies stay the JSON RSV-style wording.
